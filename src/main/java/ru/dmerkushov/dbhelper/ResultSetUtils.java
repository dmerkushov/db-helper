...
public class ResultSetUtils {
	...
	/**
	 * Put the contents of a ResultSet to a DOM document. Does not save the
	 * position in the ResultSet.<br>
	 * <br>
	 ...
	 * </pre>
	 *
	 * @param rs The ResultSet. The method will try to go it through, from the
	 * beginning to the end, but if the JDBC driver doesn't support {@link java.sql.ResultSet#beforeFirst()
	 * } method, or a SQLException happens, will begin at the next row after the
	 * current one
	 * @return
	...
	public static Document resultSetToDomDocument (ResultSet rs) throws DbHelperException, SQLException {
		DbHelper.loggerWrapper.entering (rs);
			...
			for (int columnIndex = 1; columnIndex <= columnCount; columnIndex++) {
				...
				switch (rsMeta.getColumnType (columnIndex)) {
					...
					case java.sql.Types.TIMESTAMP:
						...
						break;
					default:
						DbHelper.loggerWrapper.warning ("Unknown type of column " + columnIndex + ": " + rsMeta.getColumnTypeName (columnIndex) + ". Will set the value to null in the DOM document.");
		...
		DbHelper.loggerWrapper.exiting (document);
		return document;
	}
}
